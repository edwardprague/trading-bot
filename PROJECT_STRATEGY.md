...
Each chat in the Project has a single focused domain. Start every chat with "Please read CONTEXT.md" if using AL, or paste the relevant section of PROJECT_STRATEGY.md for Claude context.

### Infrastructure Chat
Dashboard, Flask server, UI improvements, AL fixes. The system is largely complete — use this chat for maintenance and new dashboard features.
...
## Workflow Reminders
- **AL** = Cowork (the agentic coding assistant)
- Keep AL sessions short — one task per session
- Always start AL with "Please read CONTEXT.md"
- After meaningful progress — update both CONTEXT.md and PROJECT_STRATEGY.md
- This chat = strategy thinking. AL = implementation. Terminal = execution.

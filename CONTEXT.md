...
## Terminology
- **AL** — refers to Cowork (the agentic coding assistant). Always use "AL" not "Cowork" going forward.
...
2. **AL** — writing and editing Python files, implementing changes (keep sessions short, one task per session)
3. **Terminal** — running backtests, git operations, direct code fixes

Always begin AL sessions with: *"Please read CONTEXT.md. [single focused task]."*

---

## Project Chat Structure (Claude Project)
Organised as separate focused chats:
- **Infrastructure** — dashboard, Flask server, UI, AL fixes
